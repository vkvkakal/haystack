...
import haystack.modeling.model.colbert.utils.distributed as distributed
from haystack.modeling.model.colbert.utils.runs import Run
from haystack.modeling.model.colbert.utils.utils import print_message, timestamp, create_directory
...
class Arguments:
    ...
    def add_compressed_index_input(self):
        self.add_argument("--compression_level", dest="compression_level", choices=[1, 2], type=int, default=None)
    ...
    def parse(self):
        ...
        if args.nranks > 1:
            print_message(
                f"#> Restricting number of threads for FAISS to {args.nthreads} per process", condition=(args.rank == 0)
            )
