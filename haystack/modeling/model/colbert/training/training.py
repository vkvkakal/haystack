...
from haystack.modeling.model.colbert.infra import ColBERTConfig
from haystack.modeling.model.colbert.training.rerank_batcher import RerankBatcher

from haystack.modeling.model.colbert.utils.amp import MixedPrecisionManager
from haystack.modeling.model.colbert.training.lazy_batcher import LazyBatcher
from haystack.modeling.model.colbert.parameters import DEVICE

from haystack.modeling.model.colbert.modeling.colbert import ColBERT
from haystack.modeling.model.colbert.modeling.reranker.electra import ElectraReranker

from haystack.modeling.model.colbert.utils.utils import print_message
from haystack.modeling.model.colbert.training.utils import print_progress, manage_checkpoints
    ...
    if collection is not None:
        if config.reranker:
            reader = RerankBatcher(
                config, triples, queries, collection, (0 if config.rank == -1 else config.rank), config.nranks
            )
        else:
            reader = LazyBatcher(
                config, triples, queries, collection, (0 if config.rank == -1 else config.rank), config.nranks
